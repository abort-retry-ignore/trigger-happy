...
- **Opposite color mode** — the crosshair auto-inverts against whatever is underneath (CSS difference blending), so it's always visible; color and outline are ignored in this mode
...
- **Keyboard shortcut** — `Alt+Shift+S` toggles the crosshair overlay instantly (set it once in `chrome://extensions/shortcuts`)
...
The toggle hotkey (`Alt+Shift+S` by default) is **not assigned automatically** on modern Chrome (138+ stopped applying `suggested_key` on install) — and reloading an unpacked extension can drop an existing binding. To set it:

1. Open `chrome://extensions/shortcuts`
2. Find **Trigger Happy → Toggle the crosshair overlay**
3. Click the input box and press `Alt+Shift+S` (or any combo you prefer)

The hotkey toggles the **crosshair overlay only** — it never disables the extension. To turn the extension itself on/off, use the master switch in the popup header. When the crosshair is hidden by the hotkey, the popup shows a banner with a one-click **Show** button.
...
4. Enable **Color → Opposite color** for a crosshair that auto-inverts against the background — ideal for games with mixed lighting

> Note: on a perfectly mid-gray background the inverted color is also mid-gray — the one case where opposite mode can't help.
...
│   ├── background/background.js  # service worker: defaults, badge, Alt+Shift+S toggle
